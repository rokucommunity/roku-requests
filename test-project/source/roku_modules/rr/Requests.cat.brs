...
function rr_Requests() as Object
    return {
        request : rr_Requests_request
        get: rr_Requests_getRequest
        post: rr_Requests_postRequest
    }
end function

function rr_Requests_getRequest(url as String, args=invalid)
    return m.request("GET", url, args)
end function

function rr_Requests_postRequest(url as String, args=invalid)
    return m.request("POST", url, args)
end function

function rr_Requests_request(method, url as String, args as Object)
    ...
    requestHeaders = rr_Requests_headers()
    requestHeaders.addHeadersAA(_headers)

    requestQueryString = rr_Requests_queryString()
    ...
    rc = rr_Requests_cache(method, url, headers)
    ...
    if response = invalid
        response = rr_Requests_run(method, url, headers, data, _timeout, _retryCount, _verify)
        if rc <> invalid and _useCache
            rc.put(response)
        end if
    end if
...
function rr_Requests_run(method, url, headers, data, timeout, retryCount, verify)

    urlTransfer = rr_RequestsUrlTransfer(true, true, verify)
    ...
    return rr_Requests_response(urlTransfer, responseEvent, requestDetails)

end function



function rr_Requests_cache(method as String, url as String, headers as Object)
...
function rr_Requests_queryString()
    ...
    return {
        ...
        addString: function(params as String)
                ...
                else if rr_Requests_Utils_inString("=", params)
                    split_params = params.split("=")
        ...
        append: function(url as String) as String
            ' "append the QS on a provided URL"
                if m._qs_array.Count() > 0
                    if rr_Requests_Utils_inString("?", url)
                        if url.right(1) = "?"
                            return url + m.build()
                        else
                            return url + "&" + m.build()
...
function rr_Requests_response(urlTransfer as Object, responseEvent as Object, requestDetails as Object)
...
function rr_RequestsUrlTransfer(EnableEncodings as Boolean, retainBodyOnError as Boolean, verify as String)
...
function rr_Requests_Utils_inString(char as String, strValue as String)
