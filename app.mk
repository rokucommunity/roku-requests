...
ifdef ROKU_DEVPASSWORD
    USERPASS = rokudev:$(ROKU_DEVPASSWORD)
else
    USERPASS = rokudev
endif

PKG_DEVPASSWD ?= invalidkey
...
verify-keys:
	...
	@if [ "$(HTTPSTATUS)" == " 401" ]; \
  	then \
		ROKU_DEV_ID=$(shell $(CURLCMD) --user $(USERPASS) --digest http://$(ROKU_DEV_TARGET)/plugin_package | grep $(APPDEVID) -c); \
		if [ "$$ROKU_DEV_ID" == "0" ]; then echo "ROKU_DEV_ID doesn't match $(APPDEVID)"; exit 1; fi \
	else \
		ROKU_DEV_ID=$(shell $(CURLCMD) http://$(ROKU_DEV_TARGET)/plugin_package | grep $(APPDEVID) -c); \
		if [ "$$ROKU_DEV_ID" == "0" ]; then echo "ROKU_DEV_ID doesn't match $(APPDEVID)"; exit 1; fi \
	fi
...
pkg: install verify-keys
	...
	@if [ "$(HTTPSTATUS)" == " 401" ]; \
	then \
		$(CURLCMD) --user $(USERPASS) --digest -Fmysubmit=Package -Fapp_name=$(APPNAME)/$(VERSION) -Fpasswd=$(ROKU_PKG_PASSWORD) -Fpkg_time=`expr \`date +%s\` \* 1000` "http://$(ROKU_DEV_TARGET)/plugin_package" | grep 'pkgDiv.innerHTML' | sed 's/.*href=\"\([^\"]*\)\".*/\1/' | sed 's#pkgs//##' | xargs -I{} $(CURLCMD) --user $(USERPASS) --digest -o $(PKGREL)/$(APPNAME)_$(APPDEVID)_`date +%F-%T`.pkg http://$(ROKU_DEV_TARGET)/pkgs/{} ; \
	else \
		$(CURLCMD) -Fmysubmit=Package -Fapp_name=$(APPNAME)/$(VERSION) -Fpasswd=$(ROKU_PKG_PASSWORD) -Fpkg_time=`expr \`date +%s\` \* 1000` "http://$(ROKU_DEV_TARGET)/plugin_package" | grep 'pkgDiv.innerHTML' | sed 's/.*href=\"\([^\"]*\)\".*/\1/' | sed 's#pkgs//##' | xargs -I{} $(CURLCMD) -o $(PKGREL)/$(APPNAME)_$(APPDEVID)_`date +%F-%T`.pkg http://$(ROKU_DEV_TARGET)/pkgs/{} ; \
	fi
...
SCREENSHOT_TIME := $(shell date "+%s")
screenshot-jpg:
	curl -m 1 -o screenshot.jpg --user $(USERPASS) --digest "http://$(ROKU_DEV_TARGET)/pkgs/dev.jpg?time=$(SCREENSHOT_TIME)" -H 'Accept: image/webp,image/apng,image/*,*/*;q=0.8' -H 'Accept-Language: en-US,en;q=0.5' -H 'Accept-Encoding: gzip, deflate'

screenshot-png:
	curl -m 1 -o screenshot.png --user $(USERPASS) --digest "http://$(ROKU_DEV_TARGET)/pkgs/dev.png?time=$(SCREENSHOT_TIME)" -H 'Accept: image/webp,image/apng,image/*,*/*;q=0.8' -H 'Accept-Language: en-US,en;q=0.5' -H 'Accept-Encoding: gzip, deflate'
